...
get_configuration_option() {
    ...
    if [ -n "$FILE" ]; then
        ...
        int main() {
            printf(\"%s\", STRINGIFY_MACRO($OPTION));
            return 0;
        }
        " > conftest$$.c

        $CC -I$INCLUDE_DIRECTORY -o conftest$$ conftest$$.c > /dev/null 2>&1
    ...
    case "$1" in
        remap_page_range)
            ...
            if [ -f conftest$$.o ]; then
                echo "#undef NV_REMAP_PAGE_RANGE_PRESENT" | append_conftest "functions"
                rm -f conftest$$.o
                return
            fi
            ...
            if [ -f conftest$$.o ]; then
                echo "#define NV_REMAP_PAGE_RANGE_PRESENT" | append_conftest "functions"
                echo "#define NV_REMAP_PAGE_RANGE_ARGUMENT_COUNT 5" | append_conftest "functions"
                rm -f conftest$$.o
                return
            fi
            ...
            if [ -f conftest$$.o ]; then
                echo "#define NV_REMAP_PAGE_RANGE_PRESENT" | append_conftest "functions"
                echo "#define NV_REMAP_PAGE_RANGE_ARGUMENT_COUNT 4" | append_conftest "functions"
                rm -f conftest$$.o
                return
            else
                echo "#error remap_page_range() conftest failed!" | append_conftest "functions"
                return
            fi
        ;;
      ...
      pci_dma_mapping_error)
        ...
        vm_operations_struct)
            ...
            if [ -f conftest$$.o ]; then
                echo "#define NV_VM_OPERATIONS_STRUCT_HAS_ACCESS" | append_conftest "types"
                rm -f conftest$$.o
            else
                echo "#undef NV_VM_OPERATIONS_STRUCT_HAS_ACCESS" | append_conftest "types"
            fi
        ;;
